...
/* ════════════════════════════════════════════════════════════════════
   ACCESSIBILITY CONTROLS — motion / contrast / text size.
   Injects a keyboard- and screen-reader-friendly panel on every page,
   persists choices to localStorage, and respects the OS "reduce motion"
   preference by default. Styles live in fx.css. Appended 2026-05-29.
   ═══════════════════════════════════════════════════════════════════ */
(function () {
  ...
  // Effective animation state shown in the toggle (OS pref is the default).
  ...
  function build() {
    ...
    function open(o) {
      ...
      if (o) { var first = panel.querySelector('input'); if (first) first.focus(); }
    }
    fab.addEventListener('click', function () { open(panel.getAttribute('aria-hidden') === 'true'); });
    document.addEventListener('keydown', function (e) { if (e.key === 'Escape') open(false); });
    document.addEventListener('click', function (e) {
      if (panel.getAttribute('aria-hidden') === 'false' && !panel.contains(e.target) && !fab.contains(e.target)) open(false);
    });
