...
> **Owner:** Wolf coordinates. Every agent has a named role below.
...
| Day | Who | What | Output |
|---|---|---|---|
| **Tue (Week 0)** publish | Lyrebird + Bee | This week's teardown ships at 09:00 PT | Blog, Twitter, LinkedIn, HN, Reddit, Substack, GitHub gist, YouTube short |
| **Tue (Week 1)** | Hawk | Pick next week's target from the rolling candidate list | `teardowns/next/target.json` |
| **Wed (Week 1)** | Wolf + Raffa | Send responsible disclosure email | `teardowns/next/disclosure.eml` saved with timestamp |
| **Thu (Week 1)** | (founder window) | Quiet day. Founder reads the email. Don't poke. | — |
| **Fri (Week 1)** | Mongoose + Owl | Run the audit. Score every finding. | `teardowns/next/findings.json` |
| **Sat (Week 1)** | Lyrebird + Bee | First draft of the report + the landing page | `teardowns/next/report.md`, `landing-pages/<slug>/page.md` |
| **Sun (Week 1)** | Cuttlefish + Bat | Visuals, hook variants, thumbnail | `teardowns/next/hooks.json`, `teardowns/next/cover.png` |
| **Mon (Week 1)** | Mantis | Review the full package against the rails below. Approve or kick back. | `teardowns/next/review.md` |
| **Tue (Week 1)** publish | Octopus | Push live across all surfaces at 09:00 PT | (next teardown's `Week 0`) |
...
Each agent has one job that week. Wolf coordinates. No one freelances.

| Agent | Role | What ships from them |
|---|---|---|
| **Hawk (scout)** | Picks the target. Reads the candidate pool, ranks by the `teardown-targets.md` criteria, picks one. | `target.json` — app name, URL, platform, founder name, public traction, risk score |
| **Wolf (planner)** | Sends the disclosure email Wednesday. Owns the calendar. Escalates to Raffa if the founder responds. | `disclosure.eml`, `briefings/YYYY-MM-DD.json` entry |
| **Mongoose** | Runs the audit Friday. Catalogs every finding with file/URL evidence. | `findings.json` — full structured findings, severity per finding, evidence path |
| **Owl (critic)** | Scores each finding for severity + clarity. Anything below `severity ≥ medium` AND `evidence: solid` gets cut. | `findings.scored.json` |
| **Lyrebird (writer)** | Writes the report in Raffa's voice. One pass to draft, one pass to tighten. | `report.md` |
| **Bee** | Builds the landing page that ranks for `[App Name] security audit`. SEO meta, OG image, capture form. | `landing-pages/<slug>/{page.md, meta.json, follow-up.md}` |
| **Bat (surveyor)** | Writes 5 hook variants for the Twitter thread, LinkedIn post, and HN title. | `hooks.json` |
| **Cuttlefish** | Designs the cover image, the YouTube short thumbnail, and the carousel visuals. | `cover.png`, `thumbnail.png`, `carousel/*.png` |
| **Starling** | Cross-checks: which hashtags + formats are spiking this week. Bat uses this when crafting hooks. | `output/trends-pulse-YYYY-MM-DD.jsonl` |
| **Mantis (reviewer)** | Reads the full package Monday. Checks the rails. Approves or kicks back with specific fixes. | `review.md` — score + go/no-go |
| **Octopus (dev)** | Pushes everything live Tuesday 09:00 PT. Handles the GitHub gist + the blog deploy + the cross-posting. | git commits, deploy logs |

If an agent has nothing to do that week (e.g., Starling if the audit is purely technical), they say so in their memory file. Silence isn't allowed — "no contribution this week" is a real result.
...
|---|---|---|
| 🔴 Critical | [1-line summary] | Mongoose |
| 🟠 High | [1-line summary] | Mongoose |
| 🟡 Medium | [1-line summary] | Owl flagged after Mongoose scan |
| ... | | |
...
- **Hawk** — picked the target [link memory entry]
- **Mongoose** — found [N] of the [N] findings [link probe report]
- **Owl** — scored every finding for severity + clarity [link sieve output]
- **Lyrebird** — wrote what you're reading
- **Bee** — built the landing page
- **Mantis** — reviewed the package Monday before publish
- **Wolf** — coordinated the week
...
[2-3 paragraphs. How Mongoose runs the audit. What tools it uses. What it does NOT do (e.g., "no active exploitation, no exfiltration, read-only access only"). Anyone should be able to read this and trust the audit was ethical.]
...
[Bee's CTA. Link to the landing page Bee built. One ask, not three.]
```
...
Every teardown ships to every surface below. Octopus owns the deploy. Each surface has its own format requirements; do not paste the same copy everywhere.

| Surface | Format | Owner | Requirement |
|---|---|---|---|
| **Lictor blog** | Long-form (1500-3000 words), the full report | Lyrebird writes, Octopus deploys | Canonical URL. All other surfaces link here. |
| **Twitter / X thread** | 14-18 tweets, hook in tweet 1, findings 2-12, fix CTA tweet 13, link to full report tweet 14 | Bat drafts hooks, Lyrebird writes the thread | Use the hook variant Bat scored highest. Image per finding (Cuttlefish). |
| **LinkedIn post** | 1200-1500 chars, framed as a case study, no thread format | Lyrebird | Tag the platform (Lovable / Bolt). Tag the founder if they cooperated. |
| **Hacker News** | Text-only "Show HN: I audited [App] with 11 AI agents. Here's what we found." | Lyrebird writes the title, Wolf submits | No URL submission — text post with the full TL;DR + link to blog at bottom. HN penalizes URL-only posts. |
| **Reddit r/programming** | Same as HN but slightly more technical framing | Wolf submits | Title is a question or a finding. "[App] leaks every user's [X] — here's why and how to prevent it." |
| **Substack newsletter** | The full teardown, plus a sidebar of "what this week taught me" | Lyrebird | Goes to the Lictor list every Tuesday 10:00 PT |
| **GitHub gist** | The full structured `findings.json` + reproducer script | Octopus | Public gist under the Lictor org. Linked from the blog. The proof-of-work. |
| **YouTube short** | ≤60 seconds. Hook, 3 findings, fix, CTA. | Cuttlefish storyboards, Lyrebird scripts, Raffa records | Vertical 9:16. Captions hardcoded. |
...
| **Lictor landing page** | `lictor.dev/audits/[app-slug]` — SEO-optimized to rank for "[App Name] security audit" | Bee | Headline matches the search query. Capture form below the fold. |
...
These are not suggestions. Break any of them and Mantis kicks the teardown back. Break them twice and Lictor loses the right to do public teardowns at all.
...
6. **No teardown of paying Lictor customers.** Obvious. Mantis checks this every Monday before approving.
...
│   ├── target.json              ← Hawk's output
│   ├── disclosure.eml           ← Wolf's email, timestamped
│   ├── findings.json            ← Mongoose's raw findings
│   ├── findings.scored.json     ← Owl's scored cut
│   ├── report.md                ← Lyrebird's final draft
│   ├── hooks.json               ← Bat's hook variants
│   ├── cover.png                ← Cuttlefish's cover image
│   ├── carousel/                ← Cuttlefish's per-finding visuals
│   ├── thumbnail.png            ← YouTube short thumbnail
│   ├── review.md                ← Mantis's Monday review
│   └── publish-log.md           ← Octopus's deploy log per surface
├── 2026-10-13-tymora/
│   └── ...
...
Plus, per teardown, Bee writes to `/Users/raffa/Lictor/landing-pages/<app-slug>/{page.md, meta.json, follow-up.md}` — same convention as GenerationAI.
